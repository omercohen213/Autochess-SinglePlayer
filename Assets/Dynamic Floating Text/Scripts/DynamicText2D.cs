...
public class DynamicText2D : MonoBehaviour
{
    ...
    IEnumerator SizeSwitch()
    {
        ...
        for (int i = 0; i < data.numberOfSizeAlternations; i++)
        {
            ...
            Vector2 newScale = new Vector2(data.sizes[nextSizeIndex],
    ...
    void SizeGradient()
    {
        ...
        textObject.transform.localScale = Vector2.Lerp(data.sizes[sizeIndex] * startScale, data.sizes[nextSizeIndex] * startScale, tSize);
        ...
        for (int i = 0; i < data.enterType.Length; i++)
        {
            ...
            if (!entered[i])
            {
                ...
                if (data.enterType[i] == EnterType.Pop)
                {
                    ...
                    Vector2 targetScale = startScale * data.popModifier;
                    textObject.transform.localScale = Vector2.Lerp(startScaleZero, targetScale, tSize);
    ...
    void Exit()
    {
        ...
        if (data.exitType == ExitType.Pop)
        {
            ...
            Vector2 targetScale = startScale * data.popModifier;
            textObject.transform.localScale = Vector2.Lerp(targetScale, startScaleZero, tSize);
