...
/// </remarks>
public class CustomGridLayoutGroup : CustomLayoutGroup
{
    /// <summary>
    ...
    public override void CalculateLayoutInputHorizontal()
    {
        ...
        SetLayoutInputForAxis(
            PadHorizontal + (cellSize.x + spacing.x) * minColumns - spacing.x,
            PadHorizontal + (cellSize.x + spacing.x) * preferredColumns - spacing.x,
            -1, 0);
    }
    ...
    public override void CalculateLayoutInputVertical()
    {
        ...
        else
        {
            ...
            int cellCountX = Mathf.Max(1, Mathf.FloorToInt((width - PadHorizontal + spacing.x + 0.001f) / (cellSize.x + spacing.x)));
            minRows = Mathf.CeilToInt(rectChildren.Count / (float)cellCountX);
        }

        float minSpace = PadVertical + (cellSize.y + spacing.y) * minRows - spacing.y;
    ...
    private void SetCellsAlongAxis(int axis)
    {
        ...
        else
        {
            if (cellSize.x + spacing.x <= 0)
                ...
                cellCountX = Mathf.Max(1, Mathf.FloorToInt((width - PadHorizontal + spacing.x + 0.001f) / (cellSize.x + spacing.x)));

            if (cellSize.y + spacing.y <= 0)
                cellCountY = int.MaxValue;
            else
                cellCountY = Mathf.Max(1, Mathf.FloorToInt((height - PadVertical + spacing.y + 0.001f) / (cellSize.y + spacing.y)));
        }
