...
///
[ExecuteAlways]
public abstract class CustomHorizontalOrVerticalLayoutGroup : CustomLayoutGroup
{
    ...
    {
        float combinedPadding = (axis == 0 ? PadHorizontal : PadVertical);
    ...
    protected void SetChildrenAlongAxis(int axis, bool isVertical)
    {
        ...
        if (alongOtherAxis)
        {
            float innerSize = size - (axis == 0 ? PadHorizontal : PadVertical);
        ...
        {
            float pos = (axis == 0 ? PadLeft : PadTop);
            ...
            if (surplusSpace > 0)
            {
                if (GetTotalFlexibleSize(axis) == 0)
                    pos = GetStartOffset(axis, GetTotalPreferredSize(axis) - (axis == 0 ? PadHorizontal : PadVertical));
